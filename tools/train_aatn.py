...
from pysot.utils.lr_scheduler_aatn import build_lr_scheduler
...
from pysot.datasets.dataset_aatn import TrkDataset
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            cur_lr = lr_scheduler.get_cur_lr()
            logger.info('{}epoch: {}{}'.format(BLUE, epoch + 1, RESET))

        tb_idx = idx
        if idx % num_per_epoch == 0 and idx != 0:
            for idx, pg in enumerate(optimizer.param_groups):
                logger.info('{}epoch {} lr {}{}'.format(BLUE, epoch + 1, pg['lr'], RESET))
                if rank == 0:
        ...
        if rank == 0:
            ...
            if (idx + 1) % cfg.TRAIN.PRINT_FREQ == 0:
                info = "{}Epoch: [{}][{}/{}] lr: {:.6f}{}".format(BLUE, epoch + 1, (idx + 1) % num_per_epoch,
                                                                  num_per_epoch, cur_lr, RESET)
                for cc, (k, v) in enumerate(batch_info.items()):
                    if cc % 2 == 0:
                        info += "\t{:s}".format(getattr(average_meter, k))
                    else:
                        info += "{}{:s}{}".format(RED, getattr(average_meter, k), RESET)
                logger.info(info)
