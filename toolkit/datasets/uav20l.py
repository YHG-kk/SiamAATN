...
def loaddata():
    path = 'F:/SOT_DataSet_sum/UAV123_20L'
    ...
    for jj in range(a):
        ...
        # ********** 【核心】确定循环上限 **********
        # 循环上限取 GT 长度和图片列表长度中较小的一个
        loop_limit = min(len(file), len(li))

        # 为了调试，如果两者不相等，打印警告
        if len(file) != len(li):
            print(
                f"⚠️ 数据警告：序列 {name_list[jj]} GT长度 ({len(file)}) 与图片数量 ({len(li)}) 不匹配。将以 {loop_limit} 为准进行处理。")
        # **********************************************

        # 处理 GT 数据 (bbox) 和 img_names (li)
        # 注意：这里您的代码只将前 loop_limit 帧的数据添加到 bbox 中是更合理的
        for ii in range(loop_limit):  # 注意这里将上限改为 loop_limit
        ...
        # 检查是否遗漏了对 li 的截断，但在您之前的代码中 li 在这里被用作了完整的列表
        # 确保 li 的长度与 bbox 相同
        li_aligned = li[:loop_limit]

        if len(bbox) != len(li_aligned):
            # 理论上这里的长度应该总是相等，因为 bbox 是由 loop_limit 决定的
            print(f"DEBUG: 长度不匹配 after loop: bbox={len(bbox)}, li_aligned={len(li_aligned)}")

        f.close()

        # ********** 【核心修复：只保留一个 append】 **********
        # 使用对齐后的数据添加到 c 列表中
        c.append({'attr': current_attr,
                  'gt_rect': bbox,
                  'img_names': li_aligned,
                  'init_rect': bbox[0],
...
#
#         # ********** 【重要：对齐最终输出的长度】 **********
#         # 确保传递给 Video 对象的 img_names 和 gt_rect 长度一致
#         c.append({'attr': [],
#                   'gt_rect': bbox,  # 此时 bbox 长度为 loop_limit
#                   'img_names': li[:loop_limit],  # 截断 li，使其长度与 bbox 一致
#                   'init_rect': bbox[0],
#                   'video_dir': name_list[jj]})
#         # *************************************************
#
