    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # ****** 增加诊断代码 ******
                # print(f"{Fore.GREEN}*** DEBUG: {video.name} ***{Style.RESET_ALL}")
                # print(f"GT Traj Length: {gt_traj.shape[0]}")
                # print(f"Tracker Traj Length: {tracker_traj.shape[0]}")
                # print(f"video.name: {video.name}")
                # **************************

                # # ********** 关键修复代码：以 GT 长度为准进行截断 **********
                # gt_len = gt_traj.shape[0]  # 2437
                # tracker_len = tracker_traj.shape[0]  # 3085
                #
                # if gt_len != tracker_len:
                #
                #     # ** 强制将 Tracker 轨迹截断或填充到 GT 轨迹的长度 **
                #     final_len = gt_len  # 2437
                #
                #     # 打印警告
                #     print(
                #         f"⚠️ 强制对齐（以 GT 为准）：序列 {video.name} 发现长度不匹配。GT={gt_len}, Tracker={tracker_len}。强制截断 Tracker 至 {final_len}")
                #
                #     # 截断 Tracker 轨迹 (3085 -> 2437)
                #     if tracker_len > final_len:
                #         tracker_traj = tracker_traj[:final_len]
                #     # 填充 Tracker 轨迹（如果 Tracker 意外短了）
                #     elif tracker_len < final_len:
                #         # 填充通常用 0 或上一帧，这里使用 0 填充
                #         padding = np.zeros((final_len - tracker_len, gt_traj.shape[1]), dtype=gt_traj.dtype)
                #         tracker_traj = np.concatenate((tracker_traj, padding), axis=0)
                #
                # # 此时，gt_traj 和 tracker_traj 的长度都为 2437
