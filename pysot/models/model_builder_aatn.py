...
from pysot.models.loss_aatn import select_cross_entropy_loss, weight_l1_loss, l1loss, IOULoss
from pysot.models.backbone.alexnet import AlexNet
from pysot.models.utile_aatn import AATN, clsandloc
import numpy as np
from pysot.datasets.anchortarget_aatn import AnchorTarget3


class ModelBuilderAATN(nn.Module):
    def __init__(self):
        super(ModelBuilderAATN, self).__init__()

        self.backbone = AlexNet().cuda()
        self.grader = AATN(cfg).cuda()
    ...
    def getcenter(self, mapp):
        ...
        size = mapp.size()[3]
        # location
        x = np.tile((cfg.TRAIN.STRIDE * (np.linspace(0, size - 1, size)) + cfg.TRAIN.MOV) - cfg.TRAIN.SEARCH_SIZE // 2,
                    size).reshape(-1)
        y = np.tile((cfg.TRAIN.STRIDE * (np.linspace(0, size - 1, size)) + cfg.TRAIN.MOV).reshape(-1,
        ...
        # Adjust anchor size to match x, y, w, h shapes
        anchor = np.zeros((x.shape[0], x.shape[1], 4))  # Ensure the dimensions match
