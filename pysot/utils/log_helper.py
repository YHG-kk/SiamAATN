...
def get_format(logger, level):
    if 'RANK' in os.environ:
        ...
        rank = 0
        WHITE = '\033[37m'
    format_str =  WHITE + '[%(asctime)s-rk{}-%(filename)s#%(lineno)3d] %(message)s'.format(rank)
...
def get_format_custom(logger, level):
    if 'RANK' in os.environ:
        ...
        rank = 0
        WHITE = '\033[37m'
    format_str = WHITE + '[%(asctime)s-rk{}-%(message)s'.format(rank)
    formatter = logging.Formatter(format_str)
    return formatter
...
def print_speed(i, i_time, n):
    ...
    msg = (
        f"{BLUE}Progress:{RESET} {WHITE}{i} / {n} [{int(i / n * 100)}%{RESET}]  "
        f"{BLUE}Speed:{RESET} {CYAN}{average_time:.3f} s/iter{RESET},  "
        f"{BLUE}ETA:{RESET} {YELLOW}{remaining_day}:{remaining_hour:02d}:{remaining_min:02d} (D:H:M){RESET}"
    )
